...
<div style="text-align: left;background-color: #000;">
	<h3 style="align-content: left;color: #228B22">生产厂家列表</h3>
</div>
...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
		...
		<td>${o.fax }</td>
		<td>${o.inspector }</td>
		<c:if test="${o.state == 0}"><td><a href="start.action?id=${o.id }&state=${o.state}">停用</a></td></c:if>
		<c:if test="${o.state == 1}"><td><a href="stop.action?id=${o.id }&state=${o.state}" style="color: #2E8B57">启用</a></td></c:if>
	</tr>
	</c:forEach>
