...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
		...
		<td>${o.contractProductNum }/${o.extCproductNum }</td>
		<td>${o.inputBy}</td>
		<td>${o.checkBy}</td>
		<td>${o.inspector}</td>
		...
		<td>${o.totalAmount}</td>
		<c:if test="${o.state == 0}"><td>草稿</td></c:if>
		<c:if test="${o.state == 1}"><td style="color: #2E8B57">已上报</td></c:if>
		<td><a href="${ctx}/cargo/contractproduct/tocreate.action?contractId=${o.id }" title="查看货物信息">[货物]</a></td>
	</tr>
	</c:forEach>
